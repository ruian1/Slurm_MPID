...
#SBATCH --job-name=submit_mpid_dlreco_mcc9jan_bnb5e19
...
#LIST_OF_LISTS=/cluster/kappa/90-days-archive/wongjiradlab/moon/1L1PSelection/filelists/dlreco_mcc9jan_bnb5e19.txt
LIST_OF_LISTS=/cluster/kappa/90-days-archive/wongjiradlab/moon/1L1PSelection/filelists/mpid_input/dlreco_mcc9jan_bnb5e19.txt

let "proc_line=${SLURM_ARRAY_TASK_ID}+1+0"

outdir=$WORKDIR/out_image/$SLURM_JOB_NAME
...
do
  ...
  singularity exec ${CONTAINER} bash -c "cd ${WORKDIR} && source run_mpid_image_gauss.sh ${THIS_FILE_ID} $outdir ${SLURM_ARRAY_TASK_ID} ${line}"
